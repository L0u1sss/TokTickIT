...
| Responsive / Visual | Browser automation at fixed viewports and 200% zoom plus human screenshot review | Verify actual CSS layout, breakpoints, 320 px overflow, card/table switching, Zen Green/warning tokens, selector layout, and 44 px touch targets | Deterministic seed data and fixed desktop/tablet/mobile/narrow-mobile viewports |
...
- Treat the stated 5 MB attachment limit as exactly `5,242,880` bytes: the exact limit is accepted and one byte over is rejected. Construct boundary buffers during the test instead of committing large binaries.
...
- Use deterministic UUID fixtures for `clientRequestId`. Verify first-create, identical replay, conflicting reuse, concurrent identical requests, and a simulated lost response followed by retry. Count persisted tickets, not only HTTP responses.
- For injected database and storage failures, assert the safe `500 INTERNAL_ERROR` envelope, absence of stack traces, SQL, Prisma details, filesystem paths, and credentials, plus the absence of partial rows, API-addressable files, or overwritten audit state. Temporary/unreferenced byte cleanup is best-effort and is verified separately from API visibility.
...
| API-06 | API / Integration | FR-04; AC-02; BR-01 | Concurrent ticket creation, per-UTC-year sequence allocation, and database uniqueness | Every successful number matches `TKT-YYYY-XXXXXX`, contains the expected UTC year, and is distinct; concurrent requests create no duplicate or partial database row | `server/tests/lab-02/tickets-create.test.ts` | Not run |
...
| API-11 | API / Integration | FR-08; FR-11; AC-06; BR-08–BR-10; BR-12 | Valid multipart upload to an owned ticket for each allowed file type | Each upload returns `201`, persists correct metadata/owner relation, appears active in detail, contributes to the active-count limit, and atomically advances the parent ticket’s `updatedAt` | `server/tests/lab-02/attachments.test.ts` | Not run |
| API-12 | API / Integration | FR-08; AC-06; BR-08–BR-10 | Invalid multipart shape/parts, empty/overlong/control-character/path-like filenames, extension/MIME/signature mismatch, empty file, size boundary, sequential sixth file, and competing uploads at the limit | Safe basename input succeeds; invalid filename returns exact `400 ATTACHMENT_FILENAME_INVALID`; the exact size limit returns `201`; every rejected request leaves no row or API-addressable file; from four active files exactly one of two competing valid uploads succeeds and final active count is five | `server/tests/lab-02/attachments.test.ts` | Not run |
| API-13 | API / Integration | FR-09; FR-11; AC-07; BR-12 | Active attachment download by its owning requester | Returns `200`; bytes, MIME, and `Content-Length` match upload; `Content-Disposition` has safe ASCII/UTF-8 filename; no redirect/storage path leaks, attachment metadata is unchanged, and the parent ticket’s `updatedAt` does not advance | `server/tests/lab-02/attachments-download.test.ts` | Not run |
| API-14 | API / Integration | FR-10; AC-08; BR-11 | `PATCH .../remove` with malformed JSON, unknown/missing field, whitespace, boundary-invalid, and valid reasons | Invalid input returns documented `400`; valid trimmed 5–500-character reason returns `200` and atomically stores removal flag, timestamp, reason, remover context, and the parent ticket’s advanced `updatedAt` | `server/tests/lab-02/attachments-remove.test.ts` | Not run |
| API-15 | API / Integration | FR-07–FR-10; AC-08; BR-10–BR-11 | Removed-attachment row/object retention, repeated removal, download blocking, and active-count release | The database row, internal storage reference, and stored object remain for Lab 2 while Ticket Detail returns only safe removed metadata; download and repeated removal return `404 ATTACHMENT_NOT_AVAILABLE`; original audit fields are unchanged and the removed item no longer counts toward five active files | `server/tests/lab-02/attachments-remove.test.ts` | Not run |
...
| API-17 | API / Integration | FR-12; AC-11; BR-14 | First create, identical replay, and conflicting `clientRequestId` reuse by the same or another requester | First request returns `201`/`replayed: false`; identical retry returns the same Ticket with `200`/`replayed: true` and unchanged `createdAt`/`updatedAt`; changed normalized content or requester returns `409`; exactly one row exists | `server/tests/lab-02/tickets-idempotency.test.ts` | Not run |
| API-18 | API / Integration | FR-12; AC-11; BR-14 | Concurrent identical requests and lost-response retry using one `clientRequestId` | A uniqueness race resolves to one creation plus replay response(s), never duplicate/error leakage; retry after discarding the first success response returns the original Ticket with its original timestamps and the database still contains one row | `server/tests/lab-02/tickets-idempotency.test.ts` | Not run |
...
| API-21 | API / Integration | FR-08–FR-10; AC-12; AC-15; BR-08–BR-11; BR-15 | Injected storage/staging/database failures during upload plus pre-stream and mid-stream download failures | Pre-stream failure returns the safe JSON `500`; mid-stream I/O failure terminates the response safely without appending JSON; no orphan metadata/API-addressable file or partial active-count change remains, temporary/unreferenced bytes are cleaned where possible, and no path/storage key leaks | `server/tests/lab-02/attachment-failures.test.ts` | Not run |
...
| UI-04 | UI Component | FR-05; AC-03; AC-09; AC-12; BR-12–BR-13 | My Tickets loading, true-empty, filtered-no-results, unrestricted empty-state probe, retryable error, desktop-table, and mobile-card states | A restricted zero-result response triggers exactly one same-requester page-1 probe with `pageSize=10`; zero unrestricted total selects Empty, a positive total selects No results, and probe failure selects Failure rather than guessing; populated results include priority and correct owned links | `client/tests/lab-02/MyTicketsPage.test.tsx` | Not run |
...
| UI-07 | UI Component | FR-08; AC-06; AC-12; BR-08–BR-10 | Attachment chooser validation, exact 5 MB boundary copy, active count, pending/success, and ambiguous-response reconciliation | Invalid files are explained; control disables while busy/at five; success refreshes Ticket Detail and its Last Updated value; an ambiguous outcome shows warning semantics, reloads metadata before offering manual retry, and never auto-retries | `client/tests/lab-02/TicketDetailPage.test.tsx` | Not run |
| UI-08 | UI Component | FR-10; AC-08; AC-12; BR-10–BR-11 | Removal dialog/reason validation, pending/failure retention, ambiguous-response reconciliation, and post-removal rendering | Invalid reason blocks submit; ambiguous failure keeps safe input and reloads metadata before another mutation; success refreshes Last Updated/removed metadata, removes download/re-remove actions, announces removal, and opens one replacement slot | `client/tests/lab-02/TicketDetailPage.test.tsx` | Not run |
...
| UI-11 | UI Component | FR-03; FR-12; AC-01; AC-11–AC-12; BR-14 | Logical create-attempt `clientRequestId` lifecycle, double activation, ambiguous/lost response retry, replay, and `409` conflict | One UUID is generated per logical submission and reused after retryable/lost response; double activation sends once; replay renders the original Ticket; `409` shows safe blocking copy, performs no automatic retry, and never reports success; a new logical submission rotates the UUID | `client/tests/lab-02/CreateTicketIdempotency.test.tsx` | Not run |
...
| UI-13 | UI Component | FR-02–FR-03; FR-05; FR-07–FR-10; AC-12; AC-15; BR-15 | Network/`500` recovery and ambiguous attachment-outcome warning across requester screens | Safe actionable copy renders with Retry where safe; create/removal input is retained; stale/private/raw error details never render; ambiguous upload/removal reconciliation uses warning text/icon and the documented warning tokens without claiming an unverified result | `client/tests/lab-02/FailureStates.test.tsx` | Not run |
...
| RV-05 | Responsive / Visual | FR-01–FR-10; AC-13–AC-14 | Every in-scope screen at 320 px and 200% browser zoom, including Zen Green/warning and forced-colors checks | No page-level overflow/clipping/hidden actions; focus remains visible; warning uses `#8A5500`/`#FFF4D6` plus text/icon; content order and functionality remain equivalent | `client/e2e/lab-02/responsive.spec.ts` | Not run |
...
| E2E-05 | End-to-End | FR-05–FR-10; AC-03–AC-08; AC-12; BR-12 | True-empty versus filtered-no-results plus recoverable list/detail/upload/download/remove failures | The exact unrestricted probe selects the correct list state; ambiguous upload/removal reloads metadata before retry/next mutation; other safe retries recover, entered reasons/files are retained where allowed, and no stale/foreign/internal data appears | `client/e2e/lab-02/state-recovery.spec.ts` | Not run |
...
| AC-11 | `clientRequestId` makes first create, replay, conflict, concurrency, double activation, and lost-response retry idempotent | UT-06, API-17, API-18, UI-11, E2E-04 |
| AC-12 | Loading, true-empty, no-results, recoverable failure, unavailable, and ambiguous-outcome states are distinct and safely recoverable | API-01, API-02, API-20, API-21, UI-01, UI-02, UI-03, UI-04, UI-06, UI-07, UI-08, UI-10, UI-11, UI-13, RV-02, RV-03, RV-04, E2E-03, E2E-04, E2E-05 |
| AC-13 | Requester Selection, Create Ticket, My Tickets, and Ticket Detail meet desktop/tablet/mobile, 320 px, 200% zoom, overflow, and touch-target requirements | RV-01, RV-02, RV-03, RV-04, RV-05 |
| AC-14 | Programmatic labels/relationships, keyboard operation, focus behavior, live announcements, modal behavior, and non-color meaning are verified automatically | UI-12, RV-05, E2E-06 |
...
| Narrow mobile | `< 768px` | 320×568 | Same information/actions without page-level horizontal overflow |

Repeat the full screen set at **200% browser zoom** in addition to the fixed-width runs. Zoom evidence must exercise real browser layout; CSS transforms or enlarged screenshots are not substitutes.
...
- [ ] Warning/ambiguous-outcome meaning includes readable text and/or an understandable icon; amber color alone never carries meaning.
...
- [ ] There is no page-level horizontal overflow, including at 320 px and 200% zoom. Only the tablet table’s labelled local scroll container may scroll horizontally.
...
|---|---|---|---|---|---|
| Development Requester Selection | Dropdown and Continue remain grouped; loading/empty/error/Retry are distinct; ticket shell is absent before selection | Content remains centred and logical; demo warning stays adjacent to the selector | One vertical stack at 390 px and 320 px; full-width 44 px controls; no overflow at 200% zoom | `docs/lab-02/evidence/requester-selection-desktop.png`<br>`docs/lab-02/evidence/requester-selection-tablet.png`<br>`docs/lab-02/evidence/requester-selection-mobile.png` | Not captured |
...
Capture screenshots with deterministic fixture data and no browser extensions, personal information, debug overlays, or secrets visible. A screenshot is supporting evidence, not a substitute for the automated assertions in `RV-01`–`RV-05`. The four screens require twelve baseline desktop/tablet/mobile screenshots; 320 px, 200% zoom, validation, empty, failure, and ambiguous-outcome warning captures supplement them.
