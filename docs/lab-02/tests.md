...
| UI-09 | UI Component | FR-01; AC-10; BR-03–BR-04 | Browser-tab `sessionStorage` restore and protected deep-link gating | Only an integer requester ID is stored; valid active ID restores after reload; missing/malformed/unknown/inactive values are cleared; deep links wait for revalidation and redirect to selection before owned data can render | `client/tests/lab-02/RequesterContext.test.tsx` | Local pass for storage restore, revalidation, and render gating within the current non-router shell; hosted client-suite step passed on CI baseline `8ef5a9c4b42fe1bf002ebed440f4109ac516ef59`. Route-specific redirects and E2E remain deferred to the Ticket routing increment |
